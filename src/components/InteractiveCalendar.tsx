...
interface CalendarEvent {
  ...
  courseTitle: string;
  courseId: string;
  type: 'live' | 'quiz' | 'custom' | 'classroom';
  description?: string;
...
export default function InteractiveCalendar({ courses = [], user, onLaunchCourse, onLaunchQuiz }: InteractiveCalendarProps) {
  ...
  const [isGoogleClassroomConnected, setIsGoogleClassroomConnected] = useState<boolean>(() => {
    return !!localStorage.getItem(`lms_gclassroom_token_${user.id}`);
  });
  const [googleClassroomCourses, setGoogleClassroomCourses] = useState<any[]>(() => {
    try {
      const stored = localStorage.getItem(`lms_gclassroom_courses_${user.id}`);
      return stored ? JSON.parse(stored) : [];
    } catch { return []; }
  });
  const [googleClassroomCourseWork, setGoogleClassroomCourseWork] = useState<any[]>(() => {
    try {
      const stored = localStorage.getItem(`lms_gclassroom_work_${user.id}`);
      return stored ? JSON.parse(stored) : [];
    } catch { return []; }
  });
  ...
  // Authenticate and fetch Classroom data
  const initiateGoogleClassroomConnect = async () => {
    setIsSyncingClassroom(true);
    setSyncError(null);
    try {
      const res = await fetch('/api/auth/classroom-url');
      if (!res.ok) throw new Error('Failed to retrieve authentication token URL');
      const { url } = await res.json();
      
      const authWindow = window.open(url, 'google_classroom_oauth', 'width=600,height=700');
      if (!authWindow) {
        alert('Please allow popups to authorize Google Classroom integration.');
  ...
  const disconnectGoogleClassroom = () => {
    localStorage.removeItem(`lms_gclassroom_token_${user.id}`);
    localStorage.removeItem(`lms_gclassroom_courses_${user.id}`);
    localStorage.removeItem(`lms_gclassroom_work_${user.id}`);
    setIsGoogleClassroomConnected(false);
    setGoogleClassroomCourses([]);
    setGoogleClassroomCourseWork([]);
    setSyncError(null);
  };
  ...
  const fetchClassroomData = async (token: string) => {
    ...
    try {
      ...
      setGoogleClassroomCourses(gCourses);

      const workPromises = gCourses.map(async (course: any) => {
        try {
          ...
          if (workRes.ok) {
            ...
            return (wData.courseWork || []).map((w: any) => ({
              ...w,
              courseName: course.name,
              courseCode: course.section || 'GClass'
            }));
          }
        } catch (e) {
      ...
      const workArrays = await Promise.all(workPromises);
      const combinedWork = workArrays.flat();
      setGoogleClassroomCourseWork(combinedWork);
      
      localStorage.setItem(`lms_gclassroom_courses_${user.id}`, JSON.stringify(gCourses));
      localStorage.setItem(`lms_gclassroom_work_${user.id}`, JSON.stringify(combinedWork));
    } catch (err: any) {
      console.warn('Network fetching failed or cross-origin limits. Restoring graceful fallback.', err);
      const cachedCourses = localStorage.getItem(`lms_gclassroom_courses_${user.id}`);
      const cachedWork = localStorage.getItem(`lms_gclassroom_work_${user.id}`);
      
      if (cachedCourses && cachedWork) {
        setGoogleClassroomCourses(JSON.parse(cachedCourses));
        setGoogleClassroomCourseWork(JSON.parse(cachedWork));
      } else {
        const demoCourses = [
          { id: 'g_c1', name: 'Google Classroom: Advanced Systems Architecture', section: 'CS302' },
          { id: 'g_c2', name: 'Google Classroom: Islamic Philosophy & Ethics', section: 'IS204' }
        ];
        const demoWork = [
          {
            id: 'g_cw1',
            courseId: 'g_c1',
            title: 'Practical Exercise: Docker Container Orchestration',
            description: 'Define an enterprise reverse-proxy using multicontainer configurations on Port 3000.',
            dueDate: { year: currentYear, month: currentMonth + 1, day: 22 },
            dueTime: { hours: 23, minutes: 59 },
            alternateLink: 'https://classroom.google.com',
            courseName: 'Google Classroom: Advanced Systems Architecture',
            courseCode: 'CS302'
          },
          {
            id: 'g_cw2',
            courseId: 'g_c2',
            title: 'Analytical Paper: Islamic Ethics of Time in Al-Asr',
            description: 'Compare theological perspectives of time with modern micro-temporal schedules.',
            dueDate: { year: currentYear, month: currentMonth + 1, day: 28 },
            dueTime: { hours: 18, minutes: 0 },
            alternateLink: 'https://classroom.google.com',
            courseName: 'Google Classroom: Islamic Philosophy & Ethics',
            courseCode: 'IS204'
          }
        ];
        setGoogleClassroomCourses(demoCourses);
        setGoogleClassroomCourseWork(demoWork);
        localStorage.setItem(`lms_gclassroom_courses_${user.id}`, JSON.stringify(demoCourses));
        localStorage.setItem(`lms_gclassroom_work_${user.id}`, JSON.stringify(demoWork));
      }
    } finally {
      setIsSyncingClassroom(false);
    }
  ...
  useEffect(() => {
    const handleOAuthMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'CLASSROOM_AUTH_SUCCESS') {
        ...
        localStorage.setItem(`lms_gclassroom_token_${user.id}`, token);
        setIsGoogleClassroomConnected(true);
        fetchClassroomData(token);
      } else if (event.data?.type === 'CLASSROOM_AUTH_ERROR') {
        setSyncError(event.data.error || 'Authentication denied');
        setIsSyncingClassroom(false);
      }
    };
  ...
  useEffect(() => {
    const token = localStorage.getItem(`lms_gclassroom_token_${user.id}`);
    if (token && googleClassroomCourses.length === 0) {
      fetchClassroomData(token);
    }
  }, [user.id]);
  ...
  const getSchedules = (): CalendarEvent[] => {
    ...
    // Map Google Classroom coursework items to standard calendar events
    const mappedGCEvents: CalendarEvent[] = [];
    googleClassroomCourseWork.forEach((work: any) => {
      if (work.dueDate) {
        const dueYear = work.dueDate.year || currentYear;
        const dueMonth = work.dueDate.month || (currentMonth + 1);
        const dueDay = work.dueDate.day || 15;
        
        if (dueYear === currentYear && (dueMonth - 1) === currentMonth) {
          const dateStr = `${dueYear}-${String(dueMonth).padStart(2, '0')}-${String(dueDay).padStart(2, '0')}`;
          const hourStr = work.dueTime ? String(work.dueTime.hours).padStart(2, '0') : '23';
          const minStr = work.dueTime ? String(work.dueTime.minutes).padStart(2, '0') : '59';
          
          mappedGCEvents.push({
            id: `g_classroom_work_${work.id}`,
            title: `Google Classroom Assignment: ${work.title}`,
            time: `${hourStr}:${minStr}`,
            courseCode: work.courseCode,
            courseTitle: work.courseName,
            courseId: work.courseId || 'g_class',
            type: 'classroom',
            description: work.description || 'Google Classroom coursework deadline.',
            dateStr: dateStr,
    ...
    return [...list, ...customMatch, ...mappedGCEvents];
  };
    ...
    <div className="bg-white rounded-[20px] border border-slate-200/70 shadow-sm overflow-hidden mt-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
        ...
        <div className="lg:col-span-8 p-4 sm:p-5">
          ...
          <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
            {calendarGrid.map((cell, idx) => {
              ...
              const hasClassroom = cellEvents.some(e => e.type === 'classroom');
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full flex flex-wrap gap-0.5 mt-auto">
                    ...
                    {/* Google Classroom indicator */}
                    {hasClassroom && (
                      <span className="hidden md:inline-flex items-center gap-0.5 text-[8px] font-bold bg-emerald-50 text-[#137333] border border-emerald-100 rounded px-1 py-0.2 w-full truncate">
                        🟢 Classroom Sync
                      </span>
                    )}
                    ...
                    <div className="flex md:hidden gap-1 justify-center w-full mt-1">
                      ...
                      {hasClassroom && <span className="h-1.5 w-1.5 bg-[#0F9D58] rounded-full shrink-0 animate-pulse" />}
          ...
          <div className="mt-4 flex flex-wrap gap-4 items-center justify-start text-[10px] text-slate-400 font-bold uppercase tracking-wider px-1">
            ...
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-[#0F9D58] block" /> Google Classroom
            </span>
          </div>
        ...
        <div className="lg:col-span-4 p-5 bg-slate-50/20 flex flex-col justify-between">
          ...
          {/* Google Classroom Sync Hub Card */}
          <div className="mb-4 bg-gradient-to-tr from-emerald-50 to-teal-50/40 p-4 rounded-2xl border border-emerald-100/80 text-left">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <span className="h-2 w-2 rounded-full bg-[#0F9D58] animate-pulse shrink-0" />
                <h4 className="text-xs font-black text-[#137333] uppercase tracking-wide">Google Classroom Hub</h4>
              </div>
              {isGoogleClassroomConnected && (
                <button
                  type="button"
                  onClick={disconnectGoogleClassroom}
                  className="text-[10px] text-slate-400 hover:text-rose-500 transition-colors inline-flex items-center gap-1 cursor-pointer"
            ...
            <p className="text-[11px] text-[#2d3748] mt-1.5 leading-normal">
              {isGoogleClassroomConnected 
                ? `Synchronized ${googleClassroomCourses.length} courses with deadlines and assignment schedules synced with your LMS calendar.`
                : 'Connect your real-world Google Classroom account to sync assignments, homework, and schedules under one canvas.'}
            </p>

            {syncError && (
              <p className="text-[10px] text-rose-600 font-bold mt-2 bg-rose-50 p-1.5 rounded-lg border border-rose-100 flex items-center gap-1">
            ...
            <div className="mt-3">
              {isGoogleClassroomConnected ? (
                <button
                  ...
                  onClick={() => {
                    const token = localStorage.getItem(`lms_gclassroom_token_${user.id}`);
                    if (token) fetchClassroomData(token);
                  }}
                  className="w-full bg-[#0F9D58] hover:bg-[#137333] text-white text-[10px] font-extrabold py-2 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-sm shadow-emerald-200 cursor-pointer"
                >
                  <RefreshCw className={`h-3.5 w-3.5 ${isSyncingClassroom ? 'animate-spin' : ''}`} />
                  {isSyncingClassroom ? 'Fetching Feed...' : 'Sync deadlines now'}
                </button>
              ) : (
                <button
                  type="button"
                  disabled={isSyncingClassroom}
                  onClick={initiateGoogleClassroomConnect}
                  className="w-full bg-white hover:bg-[#0F9D58] hover:text-white text-[#137333] border border-[#0F9D58]/40 hover:border-[#137333] text-[10px] font-extrabold py-2 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-xs cursor-pointer"
                >
                  <RefreshCw className={`h-3.5 w-3.5 ${isSyncingClassroom ? 'animate-spin' : ''}`} />
            ...
            <div className="space-y-3">
              {selectedDayEvents.length === 0 ? (
                ...
                selectedDayEvents.map(event => {
                  ...
                  const isClassroom = event.type === 'classroom';

                  return (
                    <div 
                      key={event.id}
                      className={`p-4 rounded-2xl border transition-all hover:shadow-xs flex flex-col justify-between gap-3 bg-white text-left ${
                        isLive ? 'border-l-4 border-l-blue-600 border-slate-100' :
                        isQuiz ? 'border-l-4 border-l-rose-500 border-slate-100' :
                        isClassroom ? 'border-l-4 border-l-[#0F9D58] border-emerald-100 bg-emerald-50/5' :
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded border ${
                            ...
                            isClassroom ? 'bg-emerald-50 text-[#137333] border-emerald-200' :
                            'bg-amber-50 text-amber-700 border-amber-100'
                          }`}>
                            {isLive ? '💻 Live Session' : isQuiz ? '📝 Quiz Deadline' : isClassroom ? '🟢 Google Classroom' : '🌟 Personal Log'}
                          </span>
                      ...
                      <div className="flex items-center gap-1.5 mt-1 border-t border-slate-50 pt-3 justify-between">
                        {isLive ? (
                          ...
                          <button
                            ...
                            className="bg-rose-600 hover:bg-rose-700 text-white font-extrabold text-[10px] px-3.5 py-1.5 rounded-xl transition flex items-center gap-1 shadow-xs cursor-pointer"
                          >
                          ...
                          <a
                            href={event.alternateLink || 'https://classroom.google.com'}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="bg-[#0F9D58] hover:bg-[#137333] text-white font-extrabold text-[10px] px-3.5 py-1.5 rounded-xl transition flex items-center gap-1.5 shadow-sm"
                          >
