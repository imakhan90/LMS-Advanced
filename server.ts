...
// -----------------------------------------------------
// Google Classroom OAuth & API Helper Routes
// -----------------------------------------------------

app.get('/api/auth/classroom-url', (req, res) => {
  const authUrl = 'https://accounts.google.com/o/oauth2/v2/auth';
  const clientId = process.env.VITE_GOOGLE_CLIENT_ID || process.env.GOOGLE_CLIENT_ID || "1025547036665-m3pgg2as5g2r9v7kco4p0s88f6pms845.apps.googleusercontent.com";
  
  // Construct redirect_uri dynamically based on request or config
  const protocol = req.secure || (req.headers['x-forwarded-proto'] === 'https') ? 'https' : 'http';
  const host = req.headers.host;
  const redirectUri = `${protocol}://${host}/auth/classroom-callback`;
  ...
  res.json({ url: `${authUrl}?${params.toString()}`, redirectUri });
});

// Serve OAuth Classroom Callback page
app.get(['/auth/classroom-callback', '/auth/classroom-callback/'], (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
      <head>
        ...
        <style>
          ...
          h2 { margin-bottom: 0.5rem; color: #1a202c; font-size: 1.5rem; }
          p { color: #4a5568; font-size: 0.95rem; line-height: 1.5; }
        </style>
      </head>
      <body>
        <div class="card">
          ...
          <p>We are establishing a secure connection to Google Classroom to sync your deadlines and lecture schedules.</p>
        </div>
        <script>
          function extractTokenAndPost() {
            ...
            if (hash) {
              ...
              if (token) {
                if (window.opener) {
                  window.opener.postMessage({ type: 'CLASSROOM_AUTH_SUCCESS', token: token }, '*');
                  setTimeout(() => window.close(), 1000);
                  return;
                }
              } else if (error) {
                if (window.opener) {
                  window.opener.postMessage({ type: 'CLASSROOM_AUTH_ERROR', error: error }, '*');
                  setTimeout(() => window.close(), 1000);
                  return;
                }
              }
            ...
            if (queryError && window.opener) {
              window.opener.postMessage({ type: 'CLASSROOM_AUTH_ERROR', error: queryError }, '*');
              setTimeout(() => window.close(), 1000);
            }
          }
          extractTokenAndPost();
